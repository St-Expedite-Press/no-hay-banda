...
- [x] ✅ Configure X MCP servers. — *All four candidates deployed 2026-06-01: Barresider/x-mcp (primary, x-mcp-read enabled, x-mcp-write disabled), microsoft/playwright-mcp (fallback, inherited from global), miles0sage/twitter-mcp (experimental, enabled), kitadmin01/social_mcp (reference, disabled). Engagement tools excluded from all entries at Hermes filter level. Four login.js bugs patched (selector drift, headless mode, auth path, stdio pollution). Canary tested: `twitter_user` confirmed working; Barresider auth pending — temporary X rate limit from canary testing. Retry login after rate limit clears. See: `docs/30-operations/x-mcp-test-log.md`.*
